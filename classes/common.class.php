...
class Smart_SEO_Tool_Common extends Smart_SEO_Tool_Base
{
		...
		if ( is_home() || is_front_page() ) {
			...
			if ( $list_keywords && is_object( $authordata ) ) {
				...
				if ( $top_words['keywords'] ) {
                    ...
                    if($desc){
                        $desc = str_replace('，主要负责%list_keywords%等内容发布','',$desc);
                    }
                }
            ...
            */
			...
			if ($list_keywords &&  $wp_query->found_posts ) {
				...
				if ( $col ) {
                    ...
                    if($desc){
                        $desc = str_replace('，包括%list_keywords%等内容','',$desc);
                    }
				}
	...
	public static function postSeoInfo($post)
    {
        ...
        if(!$post){
            $data = [
                'page_title' => 'WordPress是一种自由和开放源代码的内容管理系统',
                'post_title' => 'WordPress是一种自由和开放源代码的内容管理系统',
                'cat_name' => '',
                'author_name' => '',
                'parent_cat' => '',
                'description' => 'WordPress是一种自由和开放源代码的内容管理系统，基于PHP和MySQL，使用WordPress，用户可以很容易地创建和维护个人或商业网站、博客、电子商务网站或应用程序，而无需深入了解编程语言或数据库管理。',
